...
void GantryControl::logicalCallback(const nist_gear::LogicalCameraImage& msg) {
    if (msg.models.size() != 0) {
...
void GantryControl::pickFromConveyor(const Product &product) {
    ...
    while (!left_gripper_status.attached) {
       ...
       pickup_pose.position.z = estimated_conveyor_pose.position.z + model_height.at(product.type) + GRIPPER_HEIGHT - EPSILON + 0.03;
